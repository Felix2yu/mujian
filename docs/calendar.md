...
- **发布范围（事件日历与 ICS 订阅）**：仅包含「正常」与「想看」状态的演出；「已取消」「未赴约」不进入日历，状态变更后会从客户端自动消失。
- **提醒事项（任务集合）**：仅「正常」状态生成提醒；「想看」「已取消」「未赴约」不生成。
...
CalDAV 账户下除了事件日历，还暴露一个**任务集合**：`https://<你的域名>/caldav/user/calendars/mujian-tasks/`（同一账户自动出现，无需单独添加）。**仅「正常」状态的演出**会变成一条提醒（「想看」「已取消」「未赴约」不生成提醒）：

- `DUE` = 演出开始时间，附带提前 12 小时的 `VALARM`，到期时由系统提醒事项原生推送通知；
- **勾选完成** → 记录被标记为「已观看/已到场」（`watched` 字段），取消勾选即清除，双向同步；
- 只支持勾选/取消勾选。在提醒事项里**删除**任务不会删除演出记录（下次同步会重新出现）；修改标题、备注等其它编辑会被服务器忽略（服务端始终以数据库为准）。

注意：Apple 提醒事项对第三方 CalDAV 任务账户的支持因系统版本而异（Thunderbird Tasks、Nextcloud Tasks 等客户端通常更稳），配置后请先真机验证勾选回写是否生效。
