...
> **变更（2026-09-14）**：前端 `/covers` 管理页已并入「数据」页（`/data`）的**「封面维护」分区**（`frontend/src/lib/components/CoverMaintenance.svelte`），导航不再单列「封面」；原 `/covers` 保留 308 跳转到 `/data`。后端接口与本文档描述的行为均未变。
