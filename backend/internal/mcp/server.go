...
func (s *Server) registerTools() {
	...
	mcp.AddTool(s.server, roTool("search_records",
		"搜索演出记录。可按关键词（匹配演出名/城市/场馆/剧团/备注/演员名等）、演员、剧目、折子、城市、分类、日期（year/month 可单独或同时使用，也支持 start/end 区间）、渠道、剧团、评分/票价区间、演出状态筛选；missing 可查字段为空的记录（数据卫生），missing_mode=any（默认，任一为空）/all（全部为空）；statuses 与 active_status 同时给出时取交集；compact=true 时每条只返回核心字段；limit/offset 分页。",
